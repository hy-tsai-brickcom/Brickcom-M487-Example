...
static const char g_NuMakerIoTM487DevModelId[] = "dtmi:brickcom:brickcom_m487_dev;1";
...
void brk_m487_reboot(int delayInSeconds)
{
 // disconnect the internet and wait 5 sec 
    _defaultSystemNetwork->disconnect();
    ThreadAPI_Sleep(delayInSeconds*1000);
    
    // power reboot..
 SYS_ResetChip(); // power-on-reset
    SYS_ResetCPU();
 NVIC_SystemReset();
}
